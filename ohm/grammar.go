...
func (g *Grammar) Metadata(k string) string {
	return g.metadata[k]
}
...
func NewGrammar(ctx context.Context, wasmBytes []byte) (*Grammar, error) {
	...
	for _, section := range customSections {
		...
		case "metadata":
			md, err := parseLEB128Strings(section.Data())
			if err != nil {
				return nil, fmt.Errorf("failed to parse version: %v", err)
			}
...
func (r *MatchResult) GetCstRoot() (Node, error) {
	g := r.grammar
	ctx := r.cstContext()
...
func (r *MatchResult) GetAllBindings() ([]Node, error) {
	g := r.grammar
	ctx := r.cstContext()
	bindingsAtFunc := g.module.ExportedFunction("bindingsAt")
	if bindingsAtFunc == nil {
